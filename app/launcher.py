...
# ── Make sure imports find the app/ directory ─────────────────
...
HEARTBEAT_TIMEOUT = 8  # seconds — if no heartbeat, browser is closed

# ── Routes (same as app.py) ───────────────────────────────────
@app.route('/')
def index():
...
# ── Heartbeat endpoint — called every 5s by the browser ──────
...
# ── Heartbeat watcher — kills server if browser goes silent ──
def watch_heartbeat():
    global last_heartbeat
